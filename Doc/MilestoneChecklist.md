...
- [x] 映射可恢复
- [x] 默认布局可重置
- [x] 已提供最小布局操作入口（切换 / 保存当前布局 / 恢复默认布局）
...
### 待补充验证
- [~] 扫描路径与最近插件恢复信息已可持久化；扫描结果列表本身仍未做独立持久化
- [ ] 扫描失败文件是否能正确记录
- [ ] 多目录搜索路径是否表现稳定
...
### 验收记录
- 当前状态：部分通过
- 备注：主路径、持久化与稳定性已完成一轮验证，且已补充最小布局操作入口（切换 / 保存当前布局 / 恢复默认布局）；完整默认布局全量回归与更完整的自定义编辑能力仍可继续补齐
...
- [x] `A/S/D/F` 能驱动该插件发声
- [~] 外部 MIDI 输入通路已接通并增加活动状态反馈；仍建议继续补充真实设备验证
- [x] 卸载插件后程序保持稳定
- [x] 键盘映射可保存并恢复
