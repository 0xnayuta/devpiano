# Archive

> 用途：说明历史存档文档的用途与当前替代文档的关系。
> 更新时机：归档文档新增、移除或替代关系变化时。

本目录保存历史文档、旧方案和已被替代的资料。

注意：归档文档中的内部链接、任务状态和路径可能已经过期。当前信息请优先参考现行文档。

| 归档文档 | 当前替代文档 |
|---|---|
| `phase5-architecture-convergence.md` | `docs/roadmap/roadmap.md`（Phase 5 摘要）、`docs/reference/architecture.md` |
| `phase2-3-implementation-backlog.md` | `docs/roadmap/roadmap.md` |
| `phase6-7-completion-detail.md` | `docs/roadmap/roadmap.md`（Phase 6-7 摘要） |
| `phase8-9-completion.md` | `docs/roadmap/current-iteration.md`（已迁移至 Phase 10） |
| `phase10-ui-modernization.md` | `docs/roadmap/roadmap.md`（Phase 10 摘要） |
| `phase11-declarative-ui-jive.md` | `docs/roadmap/roadmap.md`（Phase 11 摘要）、`docs/roadmap/current-iteration.md`（现为 Phase 13 规划） |
| `audit-001-code-quality-fix-phases.md` | `docs/roadmap/roadmap.md`（全面审计摘要）、`docs/roadmap/current-iteration.md`（现为 Phase 13 规划） |
| `architecture-optimization-backlog.md` | `docs/roadmap/roadmap.md`（架构优化摘要）、`docs/roadmap/current-iteration.md`（现为 Phase 10） |
