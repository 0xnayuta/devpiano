...
> **复审记录**：2026-07-23 — 修正 0.2 汇总表（原始审计 Ch8 实际 63 项，汇总表误写为 60）。Phase F (剩余项评估) 完成，ERR-001/REC-004/ERR-005/THREAD-004/ERR-007/ERR-008/QUAL-002/QUAL-006 → Closed，剩余 24 项经逐项分析后全部标记 Deferred（含 6 项 Won't-Fix：THREAD-001/PLUG-007/SEC-001/SEC-003/SEC-005/PERF-002，均属成本>收益或非实际问题）。审计关闭：0 Open, 24 Deferred, 39 Closed。
...
| `PLUG-004` | P1 | Closed | PluginOperationController 析构不卸载插件 | `source/Plugin/PluginOperationController.h`, `source/Plugin/PluginOperationController.cpp` | 析构函数仅取消扫描，不调用 `closePluginEditor()` 或通过 PluginHost 卸载插件 | 已确认安全（2026-08-16 复审）：editor（controller 的 unique_ptr 成员）与插件实例（PluginHost 的 unique_ptr 成员）均由 RAII 析构链自动释放；MainComponent 成员析构顺序（pluginOperationController 声明于 :195，先于 pluginHost :170 析构）保证 editor 先于插件宿主销毁，无悬垂/泄漏。析构语境下无需显式 unload（设备重建保护与 UI 状态更新仅运行时需要） |
...
| ID | Priority | Status | 标题 | 文件:行号 | 描述 | 建议修复 |
| --- | --- | --- | --- | --- | --- | --- |
| `PLUG-005` | P2 | Deferred | PluginHost::getInstance() 暴露裸指针 | `source/Plugin/PluginHost.h:64` | `getInstance()` 返回 `AudioPluginInstance*` 裸指针。调用方（AudioEngine）持有并在音频回调中使用，生命周期依赖外部协调 | 返回 `juce::AudioPluginInstance::Ptr` (引用计数智能指针) 或文档化所有权契约。2026-08-16 复审：仍为裸指针，生命周期由 runPluginActionWithAudioDeviceRebuild 外部协调 + 头文件 thread-safety contract（PLUG-001 缓解）覆盖，改造成本/收益比一般，维持 Deferred |
...
| `ERR-003` | P2 | Deferred | AppStateBuilder 仅 jassert 线程守卫 | `source/Settings/AppStateBuilder.cpp` | `buildAppStateSnapshot` 使用 `jassert(isMessageThread())` 做线程守卫——Release 构建中为 no-op | 考虑使用 `jassert` + 返回错误码（debug），或在 Release 中也保持检查。2026-08-16 复审：仍为 jassert-only（assertMessageThreadSnapshotAccess），但全部调用方（MainComponent timerCallback/UI 事件）均为消息线程，实际风险低，维持 Deferred |
...
| `ERR-006` | P2 | Deferred | SettingsStore scheduleSave 裸指针 API | `source/Settings/SettingsStore.h`, `source/Settings/SettingsStore.cpp:275-304` | `DebounceTimer` 持有 `const SettingsModel*` 裸指针（`setPayload` 存 `&m`），如果 SettingsModel 在 timer 触发前析构，使用悬垂指针 | 使用 `std::shared_ptr` 或确保 SettingsModel 生命周期长于 DebounceTimer。2026-08-16 复审：仍为裸指针 API；但全部调用方均传 MainComponent::appSettings（成员寿命长于 settingsStore 及其 saverTimer，MainComponent.h:171-172 声明顺序），且 MainComponent 析构时 settingsStore 先于 appSettings 销毁、timer 随之停止，实际无悬垂窗口；维持 Deferred |
...
| `QUAL-003` | P3 | Deferred | makeDefaultKeyboardLayout / makeFullPianoLayout 重复 | `source/Core/KeyMapTypes.h` | 两个函数中 binding 数组构建模式高度重复，每个 note 定义 ~4 行 | 提取共享的 binding 构建辅助函数。2026-08-16 复审：已提取 `makeNoteBinding` helper（每 binding 降至 1 行），两函数主体仍镜像重复（仅 base 常量与 layout id/name 不同），可通过参数化 base 合并；P3 低风险，维持 Deferred |
| `QUAL-004` | P3 | Deferred | findByKeyCode 返回裸指针 | `source/Core/KeyMapTypes.h:66` | `KeyboardLayout::findByKeyCode` 返回 `const KeyBinding*` 指向 vector 内部元素。vector 修改后指针悬垂 | 返回 `std::optional<std::reference_wrapper<const KeyBinding>>` 或索引。2026-08-16 复审：仍返回裸指针；调用方（KeyboardMidiMapper.cpp:37 等）均在 vector 未变更的同一线程、同一快照内使用，无实际悬垂路径，维持 Deferred |
| `QUAL-005` | P3 | Deferred | AudioEngine::getMidiCollector / getKeyboardState 暴露内部可变引用 | `source/Audio/AudioEngine.h:35-38` | 两个方法返回 `juce::MidiKeyboardState&` / `juce::MidiMessageCollector&` 可变引用，允许外部任意修改内部状态 | 提供 const 版本或通过专用方法暴露受限功能。2026-08-16 复审：仍存在；两个 JUCE 类型本身线程安全（MidiKeyboardState 内置 CriticalSection、MidiMessageCollector 为跨线程生产/消费设计），CustomKeyboard/键盘监听写入即 JUCE 惯用法，维持 Deferred |
...
| `SEC-006` | P3 | Deferred | KeyMapTypes 允许 aggregate init 绕过 fromClamped | `source/Core/MidiTypes.h` | `MidiNoteNumber{200}` aggregate 初始化可绕过 `fromClamped(200)` 的 clamp 保护 | 添加私有构造函数或使用 `requires` clause。2026-08-16 复审：仍为 aggregate struct（MidiNoteNumber/MidiChannel/Velocity），绕过路径存在；全项目调用点均经 fromClamped/helper 构造，无实际越界输入，维持 Deferred |
| `SEC-007` | P3 | Deferred | KeyboardMidiMapper 0/1-based channel 转换脆弱 | `source/Input/KeyboardMidiMapper.cpp` | channel 值在 0-based 和 1-based 之间手动转换，缺少类型系统保护 | 使用 `MidiChannel::toZeroBased()` 统一转换。2026-08-16 复审：triggerBinding/sendNoteOff 仍手动 `midiChannel - 1` 转换；`MidiChannel::toZeroBased()` helper 已存在（MidiTypes.h），可低成本收编，但当前路径正确且无缺陷报告，维持 Deferred |
| `SEC-008` | P3 | Closed | MidiChannelMapper::applyTransform 仅重映射 note on/off | `source/Midi/MidiChannelMapper.cpp` (applyTransform) | `applyTransform` 仅对 note on/off 消息重映射 channel，CC/pitch bend/program change 不经过矩阵路由 | 已确认设计意图并文档化（2026-08-16 复审）：MidiChannelMapper.h:7-8,26-27 明确注释 "Non-note messages (CC, pitch wheel, etc.) pass through unchanged"；且 `applyTransform` 现无任何调用方，矩阵路由实际入口为 `sendNoteOn/sendNoteOff`（接口形状即仅处理 note） |
| `PERF-004` | P3 | Deferred | KeyboardTypes::KeyboardSettings 2KB+ 固定数组 | `source/UI/KeyboardTypes.h` | `customKeyLabels`（`std::array<juce::String,128>`）和 `customKeyColours`（`std::array<juce::Colour,128>`）固定分配 ~4KB，即使未自定义也占满内存 | 改为 `std::vector` 或 sparse map（仅在少数键自定义时节省内存）。2026-08-16 复审：仍为固定数组；持久化侧已稀疏化（SettingsStore.cpp 仅存非空 label，PerformancePreset 注释说明 sparse 优化不划算），内存固定占用 ~4KB 属可接受常数，维持 Deferred |
| `PERF-005` | P3 | Deferred | KeyboardMidiMapper::isKeyCurrentlyDown O(n) 轮询 | `source/Input/KeyboardMidiMapper.cpp` | `handleKeyStateChanged` 中每帧遍历所有 binding 调用 `isKeyCurrentlyDown`（O(bindings) × O(key codes)） | 使用 `std::bitset` 或 `std::unordered_set` 替代遍历。2026-08-16 复审：仍每帧遍历 36 个 binding 轮询 OS 键状态（heldKeys 已为 unordered_set，但轮询点未变）；36 次/帧在消息线程的绝对开销可忽略，维持 Deferred |
