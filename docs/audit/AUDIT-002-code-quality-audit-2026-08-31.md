...
- [x] `TEST-001`：PluginOperationController 抽纯函数（resolvePluginScanPath、restore 计划决策）+ 提交顺序测试
...
| ERR-002 | 错误处理 | getCustomKeyboard 为空时未安全判活 | P2 | 已关闭 | 审计 | MainComponent 内多处直接访问 getCustomKeyboard 指针未判活 | `source/MainComponent.cpp:730` | - | - | 已修复：调用前全面判活与安全保护（复审 4） |
...
| QUAL-007 | 质量 | MainComponent 与 JiveUtils.h 拆树逻辑双份重复 | P3 | 已关闭 | 审计 | clearJiveStyleSheets/collectJiveComponents 在 MainComponent.cpp 匿名空间与 JiveUtils.h 完全重复；修复只改一处则另一处保留旧行为 | `source/MainComponent.cpp:78-88,100-113` vs `source/UI/jive/JiveUtils.h:15-24,27-34` | - | - | 已修复：MainComponent 移除本地副本，改用 JiveUtils.h 生产 helper（复审 2） |
| QUAL-008 | 质量 | refreshTitles 与 accessors 重复 lambda | P3 | 已关闭 | 审计 | 状态更新与标题刷新存在重复代码块 | `source/MainComponent.cpp:940` | - | - | 已修复：统一收敛消除重复（复审 2） |
