...
- **执行时机与性能优化**：编辑期 clangd 波浪线实时提示（`.clangd` 已启用 `Diagnostics.ClangTidy`，与全量同源配置）；大迭代边界跑全量 `./scripts/dev.sh tidy --all`（基于 `run-clang-tidy` 多核并行与 `tools/tidy-cache.py` 缓存加速，冷启动约 5~6 分钟，缓存命中仅约 2 秒，为唯一例行全量执行点）；**不进入 pre-commit**——日常开发依托 clangd 实时波浪线与增量 `./scripts/dev.sh tidy`（纪律见 [`../../AGENTS.md`](../../AGENTS.md) 第 3 节）。
