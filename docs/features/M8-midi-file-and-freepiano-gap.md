...
- 已定义"导入 → 回放 → 导出"的 roundtrip 边界：导入的 MIDI playback take **禁止再次导出为 MIDI**；用户已有原始 `.mid` 文件，无需把导入内容再导出为 MIDI。导入 MIDI 后允许导出 WAV；VST3 插件音色离线渲染仍后置到 M6-6e 扩展范围。
...
- **不做范围**：不把导入 playback take 转成可再导出的 MIDI 录制 take；VST3 插件音色离线渲染仍归 M6-6e 后续计划
...
- **验收标准**：
  ...
  - [x] 导入 .mid 后可导出 WAV；VST3 插件音色离线渲染仍归 M6-6e 计划并暂时搁置
...
| 功能 | Defer 原因 |
|------|-----------|
| VST3 插件离线渲染（M6-6e） | 未在真实环境验证，应独立推进；导入 MIDI 后 WAV 导出已走现有 fallback synth 路径，VST3 插件音色离线渲染仍后置 |
