...
- 评估并定义"导入 → 回放 → 导出"的 roundtrip 边界（当前导入 playback take 不作为可导出录制 take）
...
| **MIDI roundtrip** | **未完成**（导入 playback take 当前不可导出） | **未发现** | 差距：导入 → 回放可用，但导入 → 导出闭环未作为用户功能开放 | **需重新定义** | 当前为了避免把导入 playback take 当录制 take，Export 保持 disabled；M8-2 应先明确 roundtrip 语义 |
...
| **多轨** | **部分兼容**（自动选择 note 最多的单轨；不合并全部轨道） | **未发现**（单时间线） | 差距：无完整多轨模型 | **不推荐完整多轨；M8-5 可选 merge-all** | 完整多轨超出 M8；merge-all 单 timeline 有听感风险 |
...
**M8 最小组合：MIDI 文件导入 MVP + 导入路径记忆 + roundtrip / tempo / 多轨边界验证**
...
3. **Roundtrip / tempo / 多轨边界验证**（质量保证）：确认导入播放、PPQ/timeFormat、自动选轨与导出语义边界；当前导入 playback take 不开放直接导出

### 为什么

- 三项功能共享同一实现入口（`MidiFileImporter`），增量成本极低
- 形成"打开 / 回放"最小闭环，用户价值立即可见；"导入 / 导出"roundtrip 需单独定义语义
...
**是，且已完成核心实现。** M8-1、M8-1b、M8-6、M8-7 已实现并通过人工验收；M8-2、M8-3、M8-5 仍有剩余边界或增强项。
...
- 可直接复用现有回放链路
- 用户价值立竿见影
- 与录制/导出共享内部 `RecordingTake` 结构，但导入 playback take 当前不开放导出
- 不修改任何已有数据模型
...
| 完整多轨支持 | 当前单 timeline 模型不支持；M8-5 的 merge-all 单 timeline 仅是后续可选增强，不等于完整多轨模型 |
