...
- `source/UI/ControlsPanel.cpp`：Record / Stop / Play / Import MIDI / Export MIDI / Export WAV 按钮
...
- 说明：音频设备、ADSR、插件搜索路径、键盘布局、最近 MIDI 导入路径和主窗口尺寸均有持久化；最近 MIDI 导出路径字段存在但导出默认目录尚未接入
...
| **最近导入/导出路径** | **部分实现**（导入路径已接入；导出路径字段存在但未使用） | **未发现** | 差距：导出 FileChooser 尚未复用上次导出目录 | **可选** | 低风险体验增强，M8-3 可继续补齐 |
...
- **验收标准**：选择一个 .mid 文件 → 导入 → 回放 → 听到音符；Stop 后 Export MIDI / Export WAV 保持 disabled，不把导入 playback take 当作可导出的录制 take
...
**是，且已完成核心实现。** M8-1、M8-1b、M8-6、M8-7 已实现并通过人工验收；M8-2 已收敛为“导入 playback take 禁止 MIDI 再导出”的边界规则；M8-3 仍有低风险体验项；M8-5 已搁置。
