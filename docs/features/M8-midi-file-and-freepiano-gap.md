...
### MIDI 文件导入的 tempo map 边界

- **当前设计**：导入时忽略 tempo map，使用固定 PPQ（960）和秒级时间转换
- **影响**：导入有复杂 tempo 变化的 MIDI 文件时，回放时间与原始文件可能不一致
- **是否需要解决**：M8 不需要——应明确告知用户这是当前限制，并在文档中记录
- **未来**：如需精确 tempo map 支持，需要在 `RecordingTake` 中引入 tempo 事件，或在导入时重构时间线

### 多轨 MIDI 的处理边界

- **当前设计**：只导入第一轨（track 0），忽略其他轨
- **原因**：devpiano 当前是单 timeline 模型，多轨超出 MVP 范围
- **影响**：多轨文件导入后其他轨丢失
- **是否需要解决**：M8 不需要——应在 UI 中提示用户
- **未来**：如需多轨支持，需要在 `RecordingTake` 层面引入 track 概念

### channel / velocity / sustain pedal 的处理边界

- **当前设计**：完整保留——`juce::MidiMessage` 包含完整 channel、velocity、controller 信息
- **说明**：导入时不过滤任何 channel/velocity 事件，与录制路径一致

### 是否支持非 note 事件

- **当前设计**：M8 只支持 note on/off 和基本 channel voice 消息（CC、pitch bend、program change 等）
- **sysex**：不支持，导入时丢弃
- **meta 事件**：忽略，不阻塞导入
...
| Roundtrip 验证 | 最近文件列表 | 多轨 / tempo map |
...
### 6. 下一轮最推荐给 pi-mono 的任务是什么？

**M8-1：实现 MIDI 文件导入核心**

具体提示词：

> 实现 MIDI 文件导入 MVP，目标是让用户可以通过 Import MIDI 按钮打开标准 .mid 文件并在当前播放链路回放。
>
> 具体要求：
>
> 1. 新增 `source/Recording/MidiFileImporter.h/.cpp`，实现 `importMidiFile(const juce::File& midiFile, double targetSampleRate) -> std::optional<RecordingTake>`
> 2. 使用 `juce::MidiFile::readFrom()` 读取文件
> 3. 遍历所有 track，收集 note on/off 事件（第一轨优先，其他轨忽略并在日志中记录）
> 4. 将 `juce::MidiMessage` 的时间戳（秒）转换为 sample-based `timestampSamples`（`timestampSeconds * targetSampleRate`）
> 5. 填充 `RecordingTake { sampleRate, lengthSamples, events }`
> 6. `ControlsPanel` 新增 `importMidiButton { "Import MIDI" }`，enabled 条件同 `exportMidiButton`
> 7. `MainComponent::handleImportMidiClicked()` 使用 `juce::FileChooser::launchAsync(openMode)`，默认目录使用 `SettingsModel::lastMidiImportPath`，成功后更新该字段
> 8. 导入成功后自动设置 `currentTake` 并开始回放（或提示用户点击 Play）
> 9. 错误处理：空文件、无 note、读取失败等应返回错误信息，不崩溃
> 10. 不修改任何现有录制/回放/导出链路
>
> 验收标准：
> - 导入一个单轨 .mid 文件 → 点击 Play → 听到音符
> - 导入空 .mid 文件 → 显示"文件不含可演奏事件"
> - 导入多轨 .mid → 日志记录"已导入第 1 轨，N 轨已忽略"，不崩溃
> - `./scripts/dev.sh wsl-build --configure-only` 通过
> - `./scripts/dev.sh win-build` 通过
>
> 不做范围：编辑、tempo map、多轨、sysex、导出确认、UI 细节（错误提示可先只用 Logger）

---

## 12. 下一轮推荐提示词

以下提示词可直接发给 pi-mono，用于执行 M8-1：
...
```
实现 MIDI 文件导入 MVP（M8-1），目标是让用户可以通过 Import MIDI 按钮打开标准 .mid 文件并在当前播放链路回放。

## 具体实现要求

### 1. 新增 MidiFileImporter 模块

在 `source/Recording/` 下新增 `MidiFileImporter.h` 和 `MidiFileImporter.cpp`：

```cpp
namespace devpiano::recording {

// 导入结果：成功返回 RecordingTake，失败返回空
std::optional<RecordingTake> importMidiFile(const juce::File& midiFile,
                                            double targetSampleRate);

// 导入选项（第一版可极简）
struct MidiImportOptions {
    bool ignoreOtherTracks = true;  // 第一版固定 true
    int preferTrack = 0;            // 第一版固定 0
};
...
- 使用 `juce::MidiFile::readFrom()` 读取文件
- 调用 `midiFile.setTicksPerQuarterNote(960)`（如果未设置）
- 遍历 `midiFile.getTrack(0)`（只处理第一轨），收集 `noteOn` / `noteOff` 事件
- 其他轨忽略，在 Logger 中记录 "MidiFileImporter: imported track 0, N other tracks ignored"
- 将 `juce::MidiMessage::getTimeStamp()`（秒）转换为 `timestampSamples`：`int64_t(ts * targetSampleRate)`
- 填充 `RecordingTake { sampleRate = targetSampleRate, lengthSamples = lastEventTimestampSamples, events = [...] }`
- 如果文件不含 note on/off 事件，返回 `std::nullopt`

### 3. ControlsPanel 新增 Import MIDI 按钮

- `importMidiButton { "Import MIDI" }`，enabled 条件与 `exportMidiButton` 一致（hasTake 时可用；实际上导入不需要 take，所以始终可用更合理）
- 按钮文案："Import MIDI"
- 放在 `exportMidiButton` 旁边或下方

### 4. MainComponent 新增 handler

```cpp
void MainComponent::handleImportMidiClicked()
{
    // 获取上次导入路径（SettingsModel::lastMidiImportPath），用于 FileChooser 默认目录
    // 使用 juce::FileChooser::launchAsync(openMode, ...) 打开文件选择框
    // 选择文件后调用 MidiFileImporter::importMidiFile()
    // 成功时设置 currentTake 并更新 UI 状态
    // 失败时写 Logger（空文件、无 note、读取失败等）
}
```

### 5. SettingsModel 新增字段

```cpp
// 在 SettingsModel 结构体中新增：
juce::String lastMidiImportPath;
juce::String lastMidiExportPath;
```

同时在 `SettingsStore` 的 `save()`/`load()` 中持久化这两个字段。

### 6. 错误处理边界

- 空文件：Logger 记录 "MidiFileImporter: file is empty"，UI 可暂不处理（后续增强）
- 无 note 事件：Logger 记录 "MidiFileImporter: no note events found"，UI 暂不处理
- 读取失败：Logger 记录 "MidiFileImporter: failed to read file"，UI 暂不处理
- 不崩溃——所有错误路径都必须安全返回

### 7. 不修改范围

- `RecordingEngine` 不修改
- `MidiFileExporter` 不修改
- `WavFileExporter` 不修改
- `RecordingFlowSupport` 不修改
- `ExportFlowSupport` 不修改
- 任何现有 audio 链路不修改

### 8. 验收标准

手工验证：
1. 点击 Import MIDI → FileChooser 出现
2. 选择一个 .mid 文件 → 自动开始回放（或 Play 按钮可用）
3. 听到音符
4. 回放结束状态正确
5. 导入空 .mid → Logger 有记录，不崩溃
6. 导入多轨 .mid → Logger 有 "N other tracks ignored"，不崩溃
...
修改：
- `source/Settings/SettingsModel.h`（新增字段）
- `source/Settings/SettingsStore.cpp`（持久化新字段）
- `source/UI/ControlsPanel.h/.cpp`（新增 Import 按钮）
- `source/MainComponent.h/.cpp`（新增 handler 和成员）
