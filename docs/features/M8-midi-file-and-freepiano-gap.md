...
> 当前状态：M8 已进入实现与验收阶段；本文同时记录差距审计、已实现状态、剩余计划和边界。
...
- 已定义"导入 → 回放 → 导出"的 roundtrip 边界：导入的 MIDI playback take **禁止再次导出为 MIDI**；用户已有原始 `.mid` 文件，无需把导入内容再导出为 MIDI。导入 MIDI 后允许导出 WAV 的需求后置到 M6-6e 扩展范围，当前暂时搁置。
...
- `docs/roadmap/roadmap.md`（M0–M8 阶段状态、当前重点）
- `docs/roadmap/current-iteration.md`（M8 当前迭代状态、已完成验收与剩余方向）
...
|---|---|---|---|---|---|
| **MIDI 导入** | **已实现**（M8-1/M8-1b：导入并自动选择 note-rich track） | **未发现**（无 .mid 导入） | devpiano 已超过旧实现 | 已完成 | 复用 `RecordingTake` 与回放链路 |
| **MIDI 导出** | **已实现**（MidiFileExporter） | **不确定**（无证据） | 差距：devpiano 已超过旧实现 | 不做 | 已完成 |
| **MIDI roundtrip** | **已定义边界**（导入 playback take 禁止再导出 MIDI） | **未发现** | 差距：导入 → 回放可用；导入 → MIDI 再导出明确不作为用户功能 | **不实现 MIDI 再导出** | 用户已有原始 `.mid` 文件；Import MIDI 后 Export MIDI 保持 disabled 是预期行为。导入 MIDI 后导出 WAV 的需求归入 M6-6e，暂时搁置 |
...
| **最近导入/导出路径** | **已实现**（导入路径、导出路径均已接入） | **未发现** | devpiano 已具备轻量路径记忆 | 已完成 | M8-3 已补齐导出 FileChooser 复用上次导出目录 |
| **录制后编辑** | **未实现** | **部分**（录制时快照 config） | 差距：devpiano 完全不做编辑 | **不推荐** | 编辑器复杂度高，应 defer |
| **回放控制** | **已实现基础增强**（Record/Stop/Play/Back） | **有额外**（speed、sync/delay） | 差距：devpiano 仍无 speed/sync/delay 等高级控制 | M8-3 已完成基础 Back | 播放中点击 Back 会从开头重新播放 |
...
- **验收标准**：选择一个 .mid 文件 → 导入 → 回放 → 听到音符；Stop 后 Export MIDI / Export WAV 保持 disabled，不把导入 playback take 当作可导出的录制 take；Recording 期间 Import MIDI 禁用，Playing 期间仍可导入另一个 MIDI 并安全替换 playback
...
- **是否需要修改现有数据模型**：否
- **是否需要新增 UI**：与候选 1 合并
- **是否需要新增测试文档**：是（覆盖导入 playback take 不可导出）
- **可能涉及的 devpiano 文件**：同候选 1
- **可复用的现有模块**：`RecordingEngine::startPlayback()` + 当前导出按钮 enable/disable 状态流
- **验收标准**：导入 .mid → 回放正常；Stop 后 Export MIDI / Export WAV 仍 disabled；不提供“导入 MIDI → 再导出 MIDI”的用户路径
- **不做范围**：不把导入 playback take 转成可导出的录制 take；导入 MIDI 后导出 WAV 暂归 M6-6e 后续计划
...
**M8 最小组合：MIDI 文件导入 MVP + 导入路径记忆 + playback / tempo / 多轨边界验证**
...
3. **Playback / tempo / 多轨边界验证**（质量保证）：确认导入播放、PPQ/timeFormat、自动选轨与导出语义边界；导入 playback take 明确不开放 MIDI 再导出，导入后 WAV 导出归 M6-6e 暂时搁置
...
- **验收标准**：
  - [x] 单轨 .mid：导入 → 回放正常；Stop 后 Export MIDI / Export WAV 保持 disabled，不把导入 playback take 当作可导出录制 take
  - [x] 导入 .mid 后不提供“再导出 MIDI”的用户路径；这是预期边界而非缺陷
  - [~] 导入 .mid 后导出 WAV 是后续需求，归入 M6-6e 计划并暂时搁置
...
- **目标**：更新文档，记录 M8 能力边界
- **修改范围**：`docs/features/M8-midi-file-and-freepiano-gap.md`、`docs/testing/midi-file-import.md`、`docs/roadmap/current-iteration.md`、`docs/roadmap/roadmap.md`、文档入口引用
- **不修改范围**：源码
- **预期文件**：上述文档
- **验收标准**：文档与实现一致，无遗漏；当前仍需持续防止 roadmap / testing / features 状态漂移
- **风险**：无
- **回滚方式**：回退文档更改
...
- **建议阶段**：M8-later/backlog；不作为 M8 默认行为。
...
### M8-7：主窗口尺寸自适应与恢复（UI polish，可选）

- **目标**：首次启动时使用能恰好容纳当前主要界面内容的默认窗口尺寸；用户手动调整窗口大小后，下次启动恢复该尺寸。
- **状态**：已实现，2026-05-01 人工验收通过。
- **建议阶段**：M8-later；UI polish，不阻塞 MIDI 导入/回放兼容性修复。
- **拆分子任务**：
  - **M8-7a：首次启动 preferred window size**
...
- **当前效果**：人工验证确认本程序导出的 MIDI 文件、非 960 PPQ 文件、含 tempo meta event 文件和基础复杂 tempo map 限制均未发现明显问题。PPQ/timeFormat 修复已覆盖 M8-2 主要 playback 边界。
...
- **当前设计**：M8-1 初版只导入第一轨（track 0），忽略其他轨；M8-1b 已作为兼容性修正实现，自动选择含 note 最多的单一轨道。
- **原因**：devpiano 当前是单 timeline 模型，M8-1/M8-1b 不引入完整多轨模型；默认合并所有轨道可能在当前单乐器播放链路中显得嘈杂。当前决定：继续保留“选择 note 最多的单轨”作为最合适模式。
- **影响**：多轨文件导入后其他轨仍会被忽略；M8-1b 已解决常见 track 0 只有 tempo/meta、实际音符在后续轨道时导入无声的问题，但不会保证多轨内容完整。
- **是否需要解决**：常见 track 0 只有 meta 导致无声的问题已通过 M8-1b 解决；完整多轨仍不属于 M8-1b/M8-2。
- **未来**：M8-5 已搁置；若以后重新考虑 merge-all，应作为显式可选导入模式。完整多轨支持则需要在 `RecordingTake` 层面引入 track 概念。
...
**是，且已完成核心实现。** M8-1、M8-1b、M8-1c、M8-2、M8-3、M8-6、M8-7 已实现并通过人工验收；M8-1c 已收敛 Import MIDI 与录制 / 回放按钮状态；M8-2 已收敛为“导入 playback take 禁止 MIDI 再导出”的边界规则，并覆盖多轨、非 960 PPQ、tempo meta event 和复杂 tempo map 限制验证；M8-5 已搁置。
...
- 可直接复用现有回放链路
- 用户价值立竿见影
- 与录制/导出共享内部 `RecordingTake` 结构，但导入 playback take 明确不开放 MIDI 再导出；导入后 WAV 导出归 M6-6e 后续计划，当前搁置
- 不修改任何已有数据模型
...
| 功能 | Defer 原因 |
|------|-----------|
| VST3 插件离线渲染 / 导入 MIDI 后 WAV 导出（M6-6e） | 未在真实环境验证，应独立推进；导入 MIDI 后允许导出 WAV 暂归此后续计划 |
| 旧 .fpm 文件兼容 | 平台耦合、格式复杂 |
| 完整 MIDI 编辑器 / 钢琴卷帘 | 超出 M8 范围 |
| 完整多轨支持 | 当前单 timeline 模型不支持；M8-5 merge-all 已搁置，当前保留 note-rich 单轨选择作为默认模式 |
