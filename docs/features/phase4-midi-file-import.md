...
- **建议阶段**：明确不做（.fpm 格式耦合旧 Windows 平台代码，优先使用标准 MIDI 文件）
...
| Phase 4 | Phase 5（已完成） | Phase 6 | Phase 7+ |
|---------|---------|---------|---------|
| MIDI 导入（单向） | 架构收敛（MainComponent 1587→606 行） | 轻量演奏文件保存/打开 | 完整工程文件（.devpiano-project） |
| Roundtrip 验证 / playback 可视化增强 | 录制/回放/插件/设置/布局职责下沉 | 播放速度控制 | 多轨 / tempo map |
| 最近路径记忆 | | 最近文件列表 + 拖拽打开 | Piano roll / 事件编辑器 |
| 回放控制小增强 | | 基础 MIDI 编辑（delete notes） | 量化 / snap-to-grid |
| | | MIDI 导入非 note 事件增强 | 多语言 UI / MP4 导出 |
...
| 功能 | Defer 原因 | 归属阶段 |
|------|-----------|---------|
| VST3 插件离线渲染（Phase 3-2） | 未在真实环境验证，应独立推进；导入 MIDI 后 WAV 导出已走现有 fallback synth 路径，VST3 插件音色离线渲染仍后置 | Phase 7+ |
| 旧 .fpm 文件兼容 | 平台耦合、格式复杂 | 明确不做 |
| 完整 MIDI 编辑器 / 钢琴卷帘 | 超出 Phase 4 范围 | Phase 8+ |
| 完整多轨支持 | 当前单 timeline 模型不支持；Phase 4-6 merge-all 已搁置，当前保留 note-rich 单轨选择作为默认模式 | Phase 7+ |
| 完整工程文件（.devpiano-project） | 需要全面设计 | Phase 7+ |
| 文件拖拽打开 | UI 交互复杂度高 | Phase 6-3 |
| 最近文件列表 UI | 可作为 Phase 4 后续小增强 | Phase 6-3 |
