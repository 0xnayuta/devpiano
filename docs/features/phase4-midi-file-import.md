...
- `docs/features/phase3-recording-playback.md`（M6 设计与实现状态、验收标准）
...
- `docs/features/phase3-layout-presets.md`（M7 preset 系统现状）
...
| 功能 | Defer 原因 |
|------|-----------|
| VST3 插件离线渲染（Phase 3-5） | 设计评估已完成但未在真实 VST3 环境验证，不应并行推进 |
...
| 功能 | Defer 原因 |
|------|-----------|
| VST3 插件离线渲染（Phase 3-5） | 未在真实环境验证，应独立推进；导入 MIDI 后 WAV 导出已走现有 fallback synth 路径，VST3 插件音色离线渲染仍后置 |
