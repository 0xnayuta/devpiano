...
- 加载插件后切换音频设备设置：当前手工回归发现 `buffer size` 选项只能选择 `10 ms`，仍需继续确认这是 Windows / JUCE 音频后端限制，还是设置恢复 / fallback 路径导致的体验问题。
...
- 当前 Windows 侧手工回归中，设置窗口里的 `buffer size` 选项只能选择 `10 ms`；从现有实现与 JUCE 行为看，更像是当前音频后端 / 设备能力限制或恢复回退行为，仍需继续确认。
...
1. 继续调查音频设置里的 `buffer size` 选项为何当前只能选择 `10 ms`。
2. 在具备外部 MIDI 设备后补齐生命周期测试 6.3 与更多真实设备场景验证。
3. 观察并收敛退出阶段 Debug 告警。
4. 按需拆分扫描职责与实例生命周期职责。
5. 继续完善扫描错误记录、多目录扫描和插件恢复细节。
