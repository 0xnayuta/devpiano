...
1. **录制范围**：只录 MIDI note/CC（键盘 + MIDI 输入发出的原始事件），不录制插件参数自动化。
2. **多轨支持**：不分轨，所有事件在同一时间轴上，简化模型。
3. **文件格式**：标准 MIDI Type 1 文件（`.mid`），可直接用任何 DAW 或 MIDI 编辑器打开。
4. **编辑能力**：纯录制/回放，不做编辑（裁剪、拼接、量化等均为后续功能）。
5. **实时回放**：支持实时回放（边录边放或录完立即播放）。
6. **导出格式**：优先实现 MIDI 文件导出；WAV 离线渲染可后续补充；不实现 MP4/视频合成。
7. **回放时插件状态**：回放时由用户自行加载期望的插件；录制内容不含插件音色数据，只含 MIDI 事件。