...
> 当前状态：已完成。Phase 6-1/6-2/6-5/6-6/6-7 已实现并通过验证；Phase 6-3（最近文件列表 + 拖拽打开）已在架构优化阶段（P0-C）完成；Phase 6-4（基础 MIDI 编辑）永久搁置。
...
| 文件 | 职责 |
|------|------|
| `source/Diagnostics/Log.h` | 日志宏定义（`DP_LOG_INFO`、`DP_LOG_WARN`、`DP_LOG_ERROR`、`DP_DEBUG_LOG`、`DP_TRACE_MIDI`），纯宏头文件，无 .cpp |
...
- [x] 有录制 take 时，Save 按钮 enabled；点击后 FileChooser 打开。
- [x] 保存的 `.devpiano` 文件可被任何文本编辑器打开，内容为合法 JSON。
- [x] 保存的 JSON 包含 `version`、`format`、`sampleRate`、`lengthSamples`、`events` 字段。
- [x] 打开之前保存的 `.devpiano` 文件后，回放内容与原始录制一致。
- [x] 打开后 Logger 输出文件路径和事件数。
- [x] 打开损坏或格式错误的 `.devpiano` 文件时，Logger 输出错误信息，不崩溃。
- [x] 无录制数据时 Save 按钮 disabled。
- [x] Recording / Playing 期间 Save 和 Open 按钮均 disabled。
- [x] 保存后再次打开，路径记忆生效（FileChooser 默认定位到上次目录）。
...
- [x] 打开 `.devpiano` 或 `.mid` 文件后，最近文件列表更新。
- [x] 最近文件列表最多显示 10 条。
- [x] 点击最近文件列表项可打开对应文件。
- [x] 列表中文件不存在时，点击后提示并移除该项。
- [x] 拖拽 `.devpiano` 文件到窗口可打开。
- [x] 拖拽 `.mid` 文件到窗口可导入。
- [x] 拖拽不支持的文件类型时无反应或提示。
...
- [x] Save 按钮：有 take 且非录制/播放中时 enabled。
- [x] 保存的 `.devpiano` 文件为合法 JSON，包含完整事件数据。
- [x] Open 按钮：非录制/播放中时 enabled。
- [x] 打开 `.devpiano` 文件后回放内容与原始录制一致。
- [x] 打开损坏文件时不崩溃，Logger 输出错误。
- [x] 路径记忆：Save/Open 均记住上次目录。

### Phase 6-2：播放速度控制

- [x] 速度显示 + 增减按钮可用，范围 0.50x–2.00x。
- [x] 播放中变速立即生效。
- [x] 速度值持久化恢复。
...
- [x] 打开 `.devpiano` 文件后，最近文件列表更新。
- [x] 导入 `.mid` 文件后，最近文件列表更新。
- [x] 最近文件列表最多显示 10 条。
- [x] 同一文件重复打开时，列表中只保留一条（移到最前）。
- [x] 点击最近文件列表项可打开对应文件。
- [x] 列表中文件不存在时，点击后提示文件不存在并从列表移除。
- [x] 最近文件列表在程序重启后仍然有效。
...
- [x] 拖拽 `.devpiano` 文件到主窗口，触发打开演奏文件。
- [x] 拖拽 `.mid` 文件到主窗口，触发 MIDI 导入。
- [x] 拖拽不支持的文件类型时，无反应或提示不支持。
- [x] Recording / Playing 期间拖拽文件，忽略或提示。
...
- [x] 导入 `sustain-pedal.mid`（含 CC64 sustain）后，回放时延音踏板效果可听（依赖插件支持）。
- [x] 导入 `simple-notes.mid` 仅包含 note on/off 的 MIDI 文件时，行为与 Phase 4 一致，无回退。
- [x] Logger 输出导入的非 note 事件数量。
- [x] 导入后保存为 `.devpiano` 文件，非 note 事件正确保留。
- [x] 打开包含非 note 事件的 `.devpiano` 文件后，回放时非 note 事件正确还原。
...
- [x] Phase 3 录制/回放/MIDI 导出/WAV 导出主链路不受影响。
- [x] Phase 4 MIDI 导入/回放/自动选轨行为不受影响。
- [x] Phase 4 导入 playback take 禁止 MIDI 再导出的边界不受影响。
- [x] Phase 5 架构收敛后的模块职责边界不受影响。
- [x] 插件加载/卸载/editor 窗口行为不受影响。
- [x] 键盘映射和虚拟键盘显示行为不受影响。
- [x] Performance Preset 保存/加载/恢复行为不受影响。
