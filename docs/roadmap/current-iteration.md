...
- M8-5：合并所有轨道 note 到单一 timeline（未实现；后续如做，需决定是否作为可选导入模式）。
...
- [x] M8-1 / M8-1b / M8-6 / M8-7 已实现并通过人工验收。
- [~] M8-2 仅部分覆盖：PPQ/timeFormat 修正、轨道诊断和自动选轨已具备；“导入 → 导出 roundtrip”当前不是用户功能，因为导入 playback take 不可导出。
- [~] M8-3 仅部分完成：最近导入路径已实现；最近导出路径和“回到开头”未实现。
- [ ] M8-5 未实现：仍保持 note-rich 单轨导入，不合并所有轨道。

推荐剩余顺序：

1. 先重新定义并收敛 M8-2 验收语义（尤其导入 take 是否允许再导出）。
2. 再补齐 M8-3 的低风险部分（最近导出路径），暂缓“回到开头”。
3. 最后评估 M8-5 是否以“可选导入模式”实现，避免默认 merge-all 带来嘈杂/鼓轨/多音色问题。
