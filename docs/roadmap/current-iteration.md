...
**Phase 6-1 已完成**：演奏文件 Save/Open 核心功能。
  ...
  - **调试插曲**：首次调试时出现 `WeakReference` 访问冲突崩溃，根因是 Windows MSVC 侧 CMake 缓存未追踪源文件变更，导致新旧目标文件混链接。快速修复：删除 `build-win-msvc/CMakeCache.txt` 后重新 `win-build`。已记录至 [`../guides/troubleshooting.md`](../guides/troubleshooting.md)。
...
**Phase 6-7 已完成**：MIDI / Performance 测试夹具与最小回归样本库 ✅
  ...
  - `tests/fixtures/performance/simple-performance.json`（`.devpiano` JSON 格式）。
  ...
  - 详见 [`../reference/features/performance-persistence.md`](../reference/features/performance-persistence.md)。
...
**Phase 2-5 已完成**：插件扫描 UX 增强（本次迭代中途插入并完成）。
  ...
  - 详见 [`../reference/features/plugin-hosting.md`](../reference/features/plugin-hosting.md) Phase 2-5 节。
...
**搁置说明**：外部 MIDI 硬件依赖验证和 VST3 插件离线渲染（Phase 3-5）因当前条件暂无法测试，状态已记录至 [`../issues/known-issues.md`](../issues/known-issues.md) §1 和 §3。
