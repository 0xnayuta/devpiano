...
- **谐波钢琴 v1 已落地（Phase 12-2，2026-08-18）**：`source/Audio/PianoSynthVoice.h` 与 SineSynthVoice 并存注册，`AudioEngine::BuiltinSynthTone {sine, piano}` + `setBuiltinSynthTone` 切换（默认仍 sine，行为不变；后续阶段切默认）。合成：基频 + 2~7 次谐波按音区查表、velocity 双映射（v^1.5 响度 + 高次谐波亮度）、分音独立衰减（attack/release 沿用 ADSR 门控）；≤8 次 `std::sin`/voice、`renderNextBlock` 无堆分配无锁。导出路径接入归 12-3。
