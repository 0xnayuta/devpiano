...
逐键自定义标签（Per-Key Labels）和颜色（Per-Key Colors），在 `CustomKeyboard` 直接渲染；全局调号 + MIDI 移调开关（Key Signature + MIDI Transpose）+ per-channel Follow Key，打通"用 C 大调指法弹任何调"的核心演奏场景。

### Phase 9：配置快照与体验增强

**前置依赖**：Phase 8（依赖逐键标签/颜色和调号系统共同组成快照内容）

演奏配置快照（Performance Preset）：整合键位绑定、ChannelMatrix、KeyboardSettings、标签/颜色/调号为可命名的完整快照，支持一键切换（菜单/快捷键/录制事件）。
88 键完整钢琴键盘（基于 `CustomKeyboard` + `Viewport` 水平滚动，支持全音域 0–127）、Smooth Pitch Bend 回放质量、乐曲信息（Song Info / Metadata）编辑对话框。

### Phase 10：主窗口 UI 现代化

**前置依赖**：Phase 9（依赖 Performance Preset、全音域键盘、Smooth Pitch Bend 完成后的稳定功能基线）

对主窗口四大面板（Header / Plugin / Controls / Keyboard）进行视觉与布局现代化改造：自定义 LookAndFeel 暗黑主题（圆角/冷光高亮/微渐变）、ADSR/音量控制从水平滑动条转横向旋转旋钮 + ADSR 曲线可视化、插件面板从固定表单转为可折叠紧凑布局、虚拟钢琴键拟真渲染（渐变高光/黑键投影/白键圆角/黑键标签）、Transport 按钮图标化 + 底部状态栏、键盘高度下限保障与动态纵向空间分配。
...
| 键盘映射边界多 | 低中 | 基础映射已全量验证；Performance Preset 已补充专项回归清单。 |
