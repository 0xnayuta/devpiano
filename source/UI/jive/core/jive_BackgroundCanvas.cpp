...
void BackgroundCanvas::paint(juce::Graphics& g) {
    ...
    const auto backgroundScale = 1.0f - ((borderWidth / 2.0f) / static_cast<float>(getWidth()));
    apply(background, g, bounds);
    g.fillPath(
        shape,
        juce::AffineTransform::scale(backgroundScale, backgroundScale, bounds.getCentreX(), bounds.getCentreY()));
...
static std::array<CubicBezier, 4> getCorners(BorderRadii<float> radii, juce::Rectangle<float> bounds) {
    ...
    topRight.control1
        = topRight.start.translated(radii.topRight * relativeControlPointOffsetForNearPerfectEllipse, 0.0f);
    topRight.end = bounds.getTopRight().translated(0.0f, radii.topRight);
    topRight.control2
        = topRight.end.translated(0.0f, -radii.topRight * relativeControlPointOffsetForNearPerfectEllipse);

    CubicBezier bottomRight;
    bottomRight.start = bounds.getBottomRight().translated(0.0f, -radii.bottomRight);
    bottomRight.control1
        = bottomRight.start.translated(0.0f, radii.bottomRight * relativeControlPointOffsetForNearPerfectEllipse);
    bottomRight.end = bounds.getBottomRight().translated(-radii.bottomRight, 0.0f);
    bottomRight.control2
        = bottomRight.end.translated(radii.bottomRight * relativeControlPointOffsetForNearPerfectEllipse, 0.0f);

    CubicBezier bottomLeft;
    bottomLeft.start = bounds.getBottomLeft().translated(radii.bottomLeft, 0.0f);
    bottomLeft.control1
        = bottomLeft.start.translated(-radii.bottomLeft * relativeControlPointOffsetForNearPerfectEllipse, 0.0f);
    bottomLeft.end = bounds.getBottomLeft().translated(0.0f, -radii.bottomLeft);
    bottomLeft.control2
        = bottomLeft.end.translated(0.0f, radii.bottomLeft * relativeControlPointOffsetForNearPerfectEllipse);
...
void BackgroundCanvas::updateShape() {
    ...
    for (const auto& corner : getCorners(getBorderRadii(), getLocalBounds().toFloat())) {
        if (shape.isEmpty()) {
