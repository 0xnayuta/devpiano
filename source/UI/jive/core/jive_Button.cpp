...
static void triggerClick(juce::Button& button) {
    ...
    dynamic_cast<juce::Component&>(button).handleCommandMessage(0x2f3f4f99);
#else
    button.triggerClick();
#endif
}
...
private:
    ...
    void testDefaultSize() {
        ...
        auto& button = *dynamic_cast<jive::GuiItemDecorator&>(*parent->getChildren()[0]).toType<jive::Button>();
    ...
    void testEvents() {
        ...
        button.getButton().onClick = [&wasTriggered]() { wasTriggered = true; };
        clickEvent.trigger();
        expect(wasTriggered);
    }
};
