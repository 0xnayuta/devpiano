...
namespace juce {
const std::unordered_map<jive::ComponentInteractionState::Mouse, juce::String>
    VariantConverter<jive::ComponentInteractionState::Mouse>::map {
...
var VariantConverter<jive::ComponentInteractionState::Mouse>::toVar(
    const jive::ComponentInteractionState::Mouse& mouse) {
    return map.find(mouse)->second;
}

jive::ComponentInteractionState::Mouse
VariantConverter<jive::ComponentInteractionState::Mouse>::fromVar(const var& value) {
    if (value.isVoid()) {
        return jive::ComponentInteractionState::Mouse::dissociate;
    }

    return std::find_if(std::begin(map), std::end(map),
                        [value](auto&& pair) { return value.toString() == pair.second; })
        ->first;
}

var VariantConverter<jive::ComponentInteractionState::Keyboard>::toVar(
    const jive::ComponentInteractionState::Keyboard& mouse) {
    return map.find(mouse)->second;
}

jive::ComponentInteractionState::Keyboard
VariantConverter<jive::ComponentInteractionState::Keyboard>::fromVar(const var& value) {
    if (value.isVoid()) {
...
const std::unordered_map<jive::ComponentInteractionState::Keyboard, juce::String>
    VariantConverter<jive::ComponentInteractionState::Keyboard>::map {
