...
std::optional<Easing> fromString(const juce::String& name) {
    static const std::unordered_map<juce::String, Easing> namedEasings {
        { "linear", linear }, { "ease", inOut }, { "ease-in", in }, { "ease-out", out }, { "ease-in-out", inOut },
    };

    if (const auto result = namedEasings.find(name); result != std::end(namedEasings)) {
    ...
    if (name.startsWith("cubic-bezier(") && name.endsWith(")")) {
        ...
        const auto args
            = juce::StringArray::fromTokens(name.fromFirstOccurrenceOf("(", includeSubStringsInResults, ignoreCase)
