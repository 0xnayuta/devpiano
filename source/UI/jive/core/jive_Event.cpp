...
private:
    ...
    void testObserving() {
...

        {
            ...
            event1.onTrigger = [&wasTriggered]() { wasTriggered = true; };
            expect(!wasTriggered);
            event1.trigger();
            expect(wasTriggered);
...

        {
            ...
            auto wasTriggered = false;
            event2.onTrigger = [&wasTriggered]() { wasTriggered = true; };
            expect(!wasTriggered);
            event1.trigger();
            expect(wasTriggered);
        }
    ...
    void testTiming() {
        ...
        expect(event.getTimeLastTriggered() == timeJustBefore || event.getTimeLastTriggered() == timeJustAfter);
    }
    ...
    void testCopying() {
        ...
        auto wasTriggered = false;
        originalEvent.onTrigger = [&wasTriggered]() { wasTriggered = true; };
