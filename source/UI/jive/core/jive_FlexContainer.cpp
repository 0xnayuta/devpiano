...
private:
    void testDirection() {
        ...
        auto flexBox
            = static_cast<juce::FlexBox>(*dynamic_cast<jive::GuiItemDecorator&>(*item).toType<jive::FlexContainer>());
    ...
    void testPadding() {
        ...
        jive::Interpreter interpreter;
        auto item = interpreter.interpret(tree);
        expectEquals(item->getChildren()[0]->getComponent()->getPosition(), juce::Point<int> { 0, 0 });

        tree.setProperty("padding", "10 20 30 40", nullptr);
        expectEquals(item->getChildren()[0]->getComponent()->getPosition(), juce::Point<int> { 40, 10 });
    }
    ...
    void testNestedWidgetWithText() {
        ...
        beginTest("nested widget with text / short text added to widget");
        {
            ...
            expectEquals(boxModel.getContentBounds().getHeight(), font.getHeight());
        }
