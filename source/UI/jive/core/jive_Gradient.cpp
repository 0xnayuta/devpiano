...
bool Gradient::ColourStop::operator==(const ColourStop& other) const {
    return juce::approximatelyEqual(proportion, other.proportion) && colour == other.colour;
}
...
bool Gradient::operator==(const Gradient& other) const {
    return stops == other.stops && variant == other.variant && orientation == other.orientation
...
var VariantConverter<Array<jive::Gradient::ColourStop>>::toVar(const Array<jive::Gradient::ColourStop>& stops) {
    ...
    for (const auto& stop : stops) {
        object->setProperty(String { stop.proportion }, VariantConverter<Colour>::toVar(stop.colour));
    }
...
var VariantConverter<jive::Gradient>::toVar(const jive::Gradient& gradient) {
    ...
    object->setProperty("stops", VariantConverter<Array<jive::Gradient::ColourStop>>::toVar(gradient.stops));
    object->setProperty("gradient", VariantConverter<jive::Gradient::Variant>::toVar(gradient.variant));

    if (gradient.orientation.hasValue()) {
        jassert(!gradient.startEndPoints.hasValue());

        object->setProperty("orientation", VariantConverter<jive::Orientation>::toVar(*gradient.orientation));
    } else if (gradient.startEndPoints.hasValue()) {
        jassert(!gradient.orientation.hasValue());

        object->setProperty("start", VariantConverter<Point<float>>::toVar(gradient.startEndPoints->getStart()));
        object->setProperty("end", VariantConverter<Point<float>>::toVar(gradient.startEndPoints->getEnd()));
    }
...
String& operator<<(String& str, const jive::Gradient::ColourStop& stop) {
    return str << "jive::Gradient::ColourStop{ " << juce::String { stop.proportion } << ", " << stop.colour << " }";
}
