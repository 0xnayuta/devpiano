...
private:
    void testChildren() {
        ...
        item->insertChild(
            std::make_unique<jive::GuiItem>(std::make_unique<juce::Component>(), item->state.getChild(0), item.get()),
            -1);
        ...
        item->state.appendChild(juce::ValueTree { "Component" }, nullptr);
        item->insertChild(
            std::make_unique<jive::GuiItem>(std::make_unique<juce::Component>(), item->state.getChild(1), item.get()),
            -1);
        item->state.appendChild(juce::ValueTree { "Component" }, nullptr);
        item->insertChild(
            std::make_unique<jive::GuiItem>(std::make_unique<juce::Component>(), item->state.getChild(2), item.get()),
            -1);
    ...
    void testAddingMultipleChildrenAtOnce() {
...

        {
            ...
            children.push_back(
                std::make_unique<jive::GuiItem>(std::make_unique<juce::Component>(), juce::ValueTree { "Component" }));
            children.push_back(
                std::make_unique<jive::GuiItem>(std::make_unique<juce::Component>(), juce::ValueTree { "Component" }));
