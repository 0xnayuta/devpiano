...
template <> struct Interpolate<juce::String> {
    [[nodiscard]] auto operator()(const juce::String& start, const juce::String& end, double proportion) const {
        const auto isNumeric = [](const auto& text) { return text.containsOnly("0123456789.-"); };
...
template <typename T> struct Interpolate<juce::BorderSize<T>> {
    [[nodiscard]] auto operator()(juce::BorderSize<T> start, juce::BorderSize<T> end, double proportion) const {
...
template <typename T> struct Interpolate<juce::Array<T>> {
    [[nodiscard]] auto operator()(const juce::Array<T>& start, const juce::Array<T>& end, double proportion) const {
        auto result = end;
...
template <> struct Interpolate<juce::Grid::Px> {
    [[nodiscard]] auto operator()(const juce::Grid::Px& start, const juce::Grid::Px& end, double proportion) const {
        return juce::Grid::Px {
...
template <> struct Interpolate<juce::Grid::TrackInfo> {
    [[nodiscard]] juce::Grid::TrackInfo operator()(const juce::Grid::TrackInfo& start, const juce::Grid::TrackInfo& end,
...
template <> struct Interpolate<juce::Colour> {
...
template <> struct Interpolate<Fill> {
...
template <typename T> struct Interpolate<BorderRadii<T>> {
