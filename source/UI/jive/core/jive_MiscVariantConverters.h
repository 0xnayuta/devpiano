...
namespace juce {
template <> struct VariantConverter<Justification> {
...
template <typename ValueType> struct VariantConverter<BorderSize<ValueType>> {
...
template <> struct VariantConverter<Identifier> {
...
template <> struct VariantConverter<MouseCursor::StandardCursorType> {
...
private:
    static const inline Array<var> options { "inherit",   "none",    "default",  "wait",      "text",
                                             "crosshair", "copy",    "pointer",  "grab",      "left-right",
                                             "up-down",   "move",    "up",       "down",      "left",
                                             "right",     "up-left", "up-right", "down-left", "down-right" };
};

template <typename T> struct VariantConverter<Array<T>> {
...
template <> struct VariantConverter<StringArray> {
...
template <> struct VariantConverter<Font> {
...
template <> struct VariantConverter<DocumentWindow::TitleBarButtons> {
...
template <> class VariantConverter<Image> {
public:
    static Image fromVar(const var& v) {
        return Image { dynamic_cast<ImagePixelData*>(v.getObject()) };
    }
...
template <> class VariantConverter<RectanglePlacement> {
public:
    static RectanglePlacement fromVar(const var& v) {
        const auto tokens = StringArray::fromTokens(v.toString(), false);
        return std::accumulate(std::begin(tokens), std::end(tokens), 0, [](const auto& flags, const auto& token) {
    ...
    static var toVar(const RectanglePlacement& placement) {
        auto stringFlagPair = std::find_if(std::begin(map), std::end(map),
                                           [&placement](const auto& pair) { return placement.testFlags(pair.second); });
...
template <> class VariantConverter<Colour> {
public:
...
template <typename Arithmetic> class VariantConverter<Point<Arithmetic>> {
public:
...
template <typename Arithmetic> class VariantConverter<Rectangle<Arithmetic>> {
public:
...
template <std::size_t N> class VariantConverter<char[N]> {
public:
...
template <> class VariantConverter<URL> {
public:
