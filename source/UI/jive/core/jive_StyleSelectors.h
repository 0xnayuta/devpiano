...
public:
    ...
    [[nodiscard]] const PropertyType* findStyle(const std::unordered_map<StyleIdentifier, PropertyType>& styles) const {
        ...
        entries.erase(std::remove_if(std::begin(entries), std::end(entries),
                                     [this](auto&& entry) {
                                         return entry->first.className.isNotEmpty()
                                             && !classes.get().contains(entry->first.className);
                                     }),
                      std::end(entries));
        entries.erase(std::remove_if(std::begin(entries), std::end(entries),
                                     [this](auto&& entry) {
                                         return entry->first.type.isNotEmpty()
                                             && entry->first.type != state.getType().toString();
                                     }),
                      std::end(entries));
        entries.erase(std::remove_if(std::begin(entries), std::end(entries),
                                     [this](auto&& entry) { return !entry->first.enabled && enabled.getOr(true); }),
                      std::end(entries));
        entries.erase(std::remove_if(std::begin(entries), std::end(entries),
                                     [this](auto&& entry) {
        ...
        entries.erase(std::remove_if(std::begin(entries), std::end(entries),
                      ...
                      std::end(entries));
        entries.erase(std::remove_if(std::begin(entries), std::end(entries),
                                     [this](auto&& entry) { return entry->first.toggled && !toggled.get(); }),
                      std::end(entries));
        ...
        std::sort(std::begin(entries), std::end(entries), [this](auto&& first, auto&& second) {
...
private:
    template <typename Container> [[nodiscard]] static auto collectIterators(const Container& container) {
