...
StyleSheet::StyleSheet(juce::Component& sourceComponent, juce::ValueTree sourceState)
    ...
    style.onValueChange = [this] { updateStyles(); };
    selectors.onChange = [this] { applyStyles(); };

    const auto updateBorderWidth = [this] { backgroundCanvas.setBorderWidth(borderWidth.calculateCurrent()); };
...
[[nodiscard]] static StyleSheet* findClosestAncestorStyleSheet(const juce::Component& rootComponent) {
    for (auto* parent = rootComponent.getParentComponent(); parent != nullptr; parent = parent->getParentComponent()) {
        if (auto& properties = parent->getProperties(); properties.contains("style-sheet")) {
...
void StyleSheet::updateStyles(jive::Object& source, StyleIdentifier styleID) {
    const auto appendStyle = [&](const auto& styleProperty, auto& styles) {
        ...
        if (source.hasProperty(styleProperty) && styles.find(styleID) == std::end(styles)) {
            ...
            styles.at(styleID).onValueChange = [this] { applyStyles(); };
            styles.at(styleID).onTransitionProgressed = [this] { applyStyles(); };
        }
    };
    ...
    if (selectors.id.exists()) {
        appendNestedStyles(selectors.id.toString(), [&](auto& newID) { newID.id = selectors.id.get(); });
    }

    for (const auto& className : selectors.classes.get()) {
        appendNestedStyles(className, [&](auto& newID) { newID.className = className; });
    }

    appendNestedStyles(selectors.state.getType().toString(),
                       [&](auto& newID) { newID.type = selectors.state.getType().toString(); });
    appendNestedStyles("disabled", [&](auto& newID) { newID.enabled = false; });
    appendNestedStyles("focus", [&](auto& newID) { newID.keyboard = ComponentInteractionState::Keyboard::focus; });
    appendNestedStyles("active", [&](auto& newID) { newID.mouse = ComponentInteractionState::Mouse::active; });
    appendNestedStyles("hover", [&](auto& newID) { newID.mouse = ComponentInteractionState::Mouse::hover; });
    appendNestedStyles("checked", [&](auto& newID) { newID.toggled = true; });
}
...
void StyleSheet::updateStyles() {
    ...
    if (auto styleState = style.get(); styleState != nullptr) {
        ...
        const auto onTransitionProgressed = [this] { applyStyles(); };
...
void StyleSheet::applyStyles() {
    ...
    if (state.getType().toString().compareIgnoreCase("svg") == 0) {
        state.setProperty("fill", "#" + foreground.getColour()->toDisplayString(false), nullptr);
    }
...
private:
    ...
    void testFindingStylesInLocalStyleSheet() {
        beginTest("finding styles locally / no style");
        {
            ...
            expectEquals(styleSheet->getBackground(), jive::Fill {});
        ...
        beginTest("finding styles locally / basic");
        {
            ...
            const auto styleSheet = jive::StyleSheet::create(component, state);
            expectEquals(styleSheet->getBackground(), jive::Fill { juce::Colour { 0xFF123456 } });
            expectEquals(findCanvas(component)->getFill(), jive::Fill { juce::Colour { 0xFF123456 } });
        }
        ...
        beginTest("finding styles locally / selectors");
        {
            ...
            const auto styleSheet = jive::StyleSheet::create(component, state);
            expectEquals(styleSheet->getBackground(), jive::Fill { juce::Colour { 0xFFABCDEF } });
            expectEquals(findCanvas(component)->getFill(), jive::Fill { juce::Colour { 0xFFABCDEF } });
            ...
            state.setProperty("style",
                              ...
                              nullptr);
            expectEquals(styleSheet->getBackground(), jive::Fill { juce::Colour { 0xFFFEDCBA } });
            expectEquals(findCanvas(component)->getFill(), jive::Fill { juce::Colour { 0xFFFEDCBA } });
            ...
            expectEquals(styleSheet->getBackground(), jive::Fill { juce::Colour { 0xFFDEADED } });
            expectEquals(findCanvas(component)->getFill(), jive::Fill { juce::Colour { 0xFFDEADED } });
    ...
    void testFindingStylesInParentStyleSheets() {
        beginTest("finding styles among parents / no style");
        {
            ...
            juce::ValueTree state { "Component" };
            const auto styleSheet = jive::StyleSheet::create(component, state);
            expectEquals(styleSheet->getForeground(), jive::Fill {});
        ...
        beginTest("finding styles among parents / basic");
        {
            ...
            expectEquals(parentStyleSheet->getForeground(), jive::Fill { juce::Colour { 0xFF777444 } });
            expectEquals(styleSheet->getForeground(), jive::Fill { juce::Colour { 0xFF777444 } });
