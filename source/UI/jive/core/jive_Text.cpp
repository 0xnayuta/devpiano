...
private:
    void testText() {
...

        {
            ...
            auto& text = *dynamic_cast<jive::GuiItemDecorator&>(*item).toType<jive::Text>();
    ...
    void testJustification() {
...

        {
            ...
            expect(text.getTextComponent().getAttributedString().getJustification() == juce::Justification::topLeft);
        }
        {
            juce::ValueTree tree {
                "Text",
    ...
    void testLineSpacing() {
...

        {
            ...
            auto item = interpreter.interpret(tree);
            auto& text = *dynamic_cast<jive::GuiItemDecorator&>(*item).toType<jive::Text>();
            expectEquals(text.getTextComponent().getAttributedString().getLineSpacing(), 0.0f);

            tree.setProperty("line-spacing", 1.234f, nullptr);
            expectEquals(text.getTextComponent().getAttributedString().getLineSpacing(), 1.234f);
        }
        {
            juce::ValueTree tree {
                "Text",
            ...
            auto item = interpreter.interpret(tree);
            auto& text = *dynamic_cast<jive::GuiItemDecorator&>(*item).toType<jive::Text>();
            expectEquals(text.getTextComponent().getAttributedString().getLineSpacing(), 4.389f);
        }
    }
    ...
    void testNested() {
...

        {
            ...
            text = dynamic_cast<jive::GuiItemDecorator&>(*item).toType<jive::Text>();
            expectEquals<juce::String>(text->getTextComponent().getAttributedString().getText(), "Nested.");
            tree.appendChild(
                juce::ValueTree {
                    "Text",
                    {
    ...
    void testAutoSize() {
...

        {
            ...
            auto& item = *dynamic_cast<jive::GuiItemDecorator*>(parentItem->getChildren()[0])->toType<jive::Text>();
            const auto font
                = dynamic_cast<jive::TextComponent&>(*item.getComponent()).getAttributedString().getAttribute(0).font;

            const auto& boxModel = jive::boxModel(item);
            expectEquals(boxModel.getWidth(), std::ceil(jive::calculateStringWidth("This side up.", font)));
            ...
            expectEquals(boxModel.getHeight(), std::ceil(font.getHeight()) * 2.0f);
        ...
        }
        {
...

            {
                ...
                auto attributedString
                    = dynamic_cast<jive::GuiItemDecorator&>(*window->getChildren()[0]->getChildren()[0])
                          .toType<jive::Text>()
                          ->getTextComponent()
                          .getAttributedString();
                juce::TextLayout layout;
