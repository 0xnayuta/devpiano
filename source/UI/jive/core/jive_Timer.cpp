...
void Timer::timeChanged() {
    for (auto elapsed = FakeTime::now() - timeLastCallbackInvoked; elapsed.inMilliseconds() > interval.inMilliseconds();
         elapsed -= interval) {
...
public:
    ...
    void runTest() final {
        ...
        jive::Timer timer {
            [&invokeTimes](juce::Time now) mutable { invokeTimes.add(now); },
