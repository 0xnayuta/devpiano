...
std::optional<Transition> Transition::fromString(const juce::String& transitionString) {
    ...
    if (parts.size() > 2) {
        ...
        transition.timingFunction = easing::fromString(parts.getReference(2)).value_or(easing::linear);
        transition.delay = parseTime(parts.getReference(3)).value_or(juce::RelativeTime::seconds(0.0));
    }
