...
public:
    struct Listener {
        ...
        virtual void transitionProgressed(const juce::String& propertyName, const Transition& transition) = 0;
    };
    ...
    [[nodiscard]] Value calculateCurrent(const Value& sourceValue, const Value& targetValue,
    ...
    template <typename Value> [[nodiscard]] Value calculateCurrent() const {
        jassert(!source.isVoid() && !target.isVoid());

        using Converter = juce::VariantConverter<Value>;
        return calculateCurrent(Converter::fromVar(source), Converter::fromVar(target), commencement);
    }
...
private:
    template <typename ValueType, Inheritance, Accumulation, bool, Responsiveness> friend class Property;
