...
class ViewBuilder {
public:
    template <typename ViewType, typename... Args> [[nodiscard]] static auto makeView(Args&&... args) {
...
private:
    ...
    [[nodiscard]] static juce::var::NativeFunction makeViewBuilder(Args&&... args) {
        return [arguments = std::make_tuple(std::forward<Args>(args)...)](const auto&) {
            static constexpr auto createRawView = [](auto&&... theArgs) { return new ViewType { theArgs... }; };
...
/** Constructs the given ViewType as a ValueTree to be used either directly
    ...
    interpreted.
*/
template <typename ViewType, typename... Args> [[nodiscard]] auto makeView(Args&&... args) {
