...
static bool compare(const juce::Grid::TrackInfo& a, const juce::Grid::TrackInfo& b) {
    return (a.isAuto() == b.isAuto()) && (a.isFractional() == b.isFractional()) && (a.isPixels() == b.isPixels())
        && (a.getStartLineName() == b.getStartLineName()) && (a.getEndLineName() == b.getEndLineName())
        && (juce::approximatelyEqual(a.getSize(), b.getSize()));
}

static bool compare(const juce::Array<juce::Grid::TrackInfo>& a, const juce::Array<juce::Grid::TrackInfo>& b) {
    if (a.size() != b.size()) {
        return false;
    }
...
private:
    void testJustifyItems() {
        ...
        auto grid
            = static_cast<juce::Grid>(*dynamic_cast<jive::GuiItemDecorator&>(*item).toType<jive::GridContainer>());
        expect(grid.justifyItems == juce::Grid::JustifyItems::stretch);

        state.setProperty("justify-items", "start", nullptr);
        grid = static_cast<juce::Grid>(*dynamic_cast<jive::GuiItemDecorator&>(*item).toType<jive::GridContainer>());
    ...
    void testTemplateRows() {
        ...
        grid = static_cast<juce::Grid>(*dynamic_cast<jive::GuiItemDecorator&>(*item).toType<jive::GridContainer>());
        expectWithinAbsoluteError(static_cast<float>(item->getComponent()->getChildComponent(0)->getHeight()), 166.5f,
                                  0.5f);
        expectWithinAbsoluteError(static_cast<float>(item->getComponent()->getChildComponent(1)->getHeight()), 111.0f,
                                  0.5f);
        expectWithinAbsoluteError(static_cast<float>(item->getComponent()->getChildComponent(2)->getHeight()), 55.5f,
                                  0.5f);
    }
    ...
    void testNestedText() {
        ...
        const auto expectedWidth = std::ceil(jive::calculateStringWidth("hello", font))
            + std::ceil(jive::calculateStringWidth("world", font));
