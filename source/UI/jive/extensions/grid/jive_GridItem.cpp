...
static bool compare(juce::GridItem::StartAndEndProperty a, juce::GridItem::StartAndEndProperty b) {
    return compare(a.start, b.start) && compare(a.end, b.end);
}

static bool compare(juce::GridItem::Margin a, juce::GridItem::Margin b) {
    return juce::approximatelyEqual(a.top, b.top) && juce::approximatelyEqual(a.right, b.right)
        && juce::approximatelyEqual(a.bottom, b.bottom) && juce::approximatelyEqual(a.left, b.left);
}
...
private:
    void testComponent() {
        ...
        const auto gridItem = dynamic_cast<jive::GuiItemDecorator&>(item).toType<jive::GridItem>()->toJuceGridItem(
            {}, jive::LayoutStrategy::real);
    ...
    void testOrder() {
...

        {
            ...
            auto parent = interpreter.interpret(state);
            auto& item = *parent->getChildren()[0];
            auto gridItem = dynamic_cast<jive::GuiItemDecorator&>(item).toType<jive::GridItem>()->toJuceGridItem(
                {}, jive::LayoutStrategy::real);
            expectEquals(gridItem.order, 0);

            state.getChild(0).setProperty("order", 123, nullptr);
            gridItem = dynamic_cast<jive::GuiItemDecorator&>(item).toType<jive::GridItem>()->toJuceGridItem(
                {}, jive::LayoutStrategy::real);
