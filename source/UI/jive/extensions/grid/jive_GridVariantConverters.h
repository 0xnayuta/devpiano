...
namespace juce {
template <> struct VariantConverter<GridItem::JustifySelf> {
...
template <> struct VariantConverter<GridItem::AlignSelf> {
...
template <> struct VariantConverter<Grid::AlignItems> {
...
template <> struct VariantConverter<Grid::JustifyContent> {
...
template <> struct VariantConverter<Grid::AlignContent> {
...
template <> struct VariantConverter<Grid::AutoFlow> {
...
template <> struct VariantConverter<Grid::TrackInfo> {
    static Grid::TrackInfo fromVar(const var& v);
    static var toVar(const Grid::TrackInfo& info);
};

template <> struct VariantConverter<Grid::Px> {
