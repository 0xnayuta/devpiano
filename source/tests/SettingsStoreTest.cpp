...
SettingsModel makePopulatedModel() {
    ...
    m.customKeyLabels[5] = "hi";
    m.customKeyLabels[100] = "C";
    m.customKeyColours[10] = juce::Colour(0xff00ff00);
    m.languageCode = "zh-CN";
    m.lastMidiExportPath = "/tmp/last-export.mid";
    m.lastMidiImportPath = "/tmp/last-import.mid";
    m.resizableWindow = false;
    m.showInstrumentFilter = false;
...
public:
    ...
    void runTest() override {
        testCase("save then load restores every persisted field", [&] {
            ...
            expectEquals(loaded.customKeyLabels[5], juce::String("hi"));
            expectEquals(loaded.customKeyLabels[100], juce::String("C"));
            expect(loaded.customKeyColours[10] == juce::Colour(0xff00ff00), "colour must round-trip");
            ...
            expect(!loaded.resizableWindow);
            expect(!loaded.showInstrumentFilter);
